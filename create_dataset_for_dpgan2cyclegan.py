...
if __name__ == "__main__":
    ...
    for idm, model in enumerate(os.listdir(masks_path)):
        print(model)
        sorted_files = sorted(os.listdir(os.path.join(masks_path, model, "best", "label")), key=extract_mask_number)
        for file in tqdm(sorted_files, total=len(sorted_files)):
            ...
            # # MASK
            mask_path = f"{masks_path}/{model}/best/label/{file}"
            ...
            if idm < 1200:
                image.save(
                    f"{output_path}\\cephalo_line_{idm}\\{MAIN_FOLDER[0]}\\{SUB_FOLDERS[0]}\\{img}"
                )
                mask.save(
                    f"{output_path}\\cephalo_line_{idm}\\{MAIN_FOLDER[1]}\\{SUB_FOLDERS[0]}\\{img}"
                )
            elif 1200 <= idm < 1500:
                image.save(
                    f"{output_path}\\cephalo_line_{idm}\\{MAIN_FOLDER[0]}\\{SUB_FOLDERS[1]}\\{img}"
                )
                mask.save(
                    f"{output_path}\\cephalo_line_{idm}\\{MAIN_FOLDER[1]}\\{SUB_FOLDERS[1]}\\{img}"
                )
            else:
                image.save(
                    f"{output_path}\\cephalo_line_{idm}\\{MAIN_FOLDER[0]}\\{SUB_FOLDERS[2]}\\{img}"
                )
                mask.save(
                    f"{output_path}\\cephalo_line_{idm}\\{MAIN_FOLDER[1]}\\{SUB_FOLDERS[2]}\\{img}"
                )
        break
